...
FROM payment
GROUP BY cliente
HAVING count(*) < 25 AND SUM(amount) > 100
ORDER BY media DESC, total, numeroDeAlquileres;

·Obtener los 5 nombres de actor más repetidos (aprox).

SELECT first_name , COUNT(*) repeticiones
FROM actor
GROUP BY first_name
...
SELECT CONCAT(c.first_name, ' ' ,c.last_name) nombre, a.address, a.postal_code, ci.city, co.country
FROM customer AS c
...
SELECT s.store_id Tienda, s.first_name Nombre, s.last_name Apellido, CONCAT(ci.city, ' ' ,co.country) tienda, count(inv.film_id) 'Nº de Peliculas', COUNT(DISTINCT inv.film_id) 'Peliculas Diferentes', COUNT(DISTINCT cus.store_id) 'Nº de Clientes'
FROM staff AS s
INNER JOIN address AS a 
...
SELECT r.rental_id, fT.title Titulo, r.rental_date 'Fecha de alquiler', r.return_date 'Fecha de devolcuion', DATEDIFF(r.return_date,r.rental_date) AS tiempoAlquilado, cus.first_name 'Nombre de Cliente', cus.last_name 'Apellido de Cliente', sta.first_name 'Nombre Trabajador', sta.last_name 'Apellido Trabajador', ci.city 'Ciudad de la tienda', co.country 'Pais de la tienda'
FROM rental AS r
...
SELECT RANK() OVER(ORDER BY COUNT(*) DESC) Ranking, actor.actor_id,actor.first_name,actor.last_name, co.country 'Pais', count(r.rental_id) AS numeroDeAlquileres 
FROM actor 
...
HAVING actor.first_name LIKE "GINA" AND actor.last_name LIKE "DEGENERES"
